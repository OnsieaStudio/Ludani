 ...
 *
 */
public abstract class NormalizableWithListener<T> implements INormalizable<T>
{
	protected IListener<T>		valueListener;
	protected ISetter<T>		valueSetter;
	private final IListener<T>	maxListener;

	/**
	 * @param valueIn [NOT NORMALIZED VALUE]
	 * @param maxIn
	 */
	public NormalizableWithListener(final IListener<T> valueListenerIn, final ISetter<T> valueSetterIn,
			final IListener<T> maxListenerIn)
	{
		this.valueListener	= valueListenerIn;
		this.valueSetter	= valueSetterIn;
		this.maxListener	= maxListenerIn;
	}

	/**
	 * @param valueListenerIn
	 * @param valueSetterIn
	 * @param normalizable2dIn
	 */
	public NormalizableWithListener(final IListener<T> valueListenerIn, final ISetter<T> valueSetterIn,
			final NormalizableWithListener<T> normalizable2dIn)
	{
		this.valueListener	= valueListenerIn;
		this.valueSetter	= valueSetterIn;
		this.maxListener	= normalizable2dIn.maxListener;
	}

	@Override
	public INormalizable<T> value(final T valueIn)
	{
		this.valueSetter.set(valueIn);
	...
	public T max()
	{
		return this.maxListener.get();
	}

	@Override
	public T notNormalized()
	{
		return this.valueListener.get();
	}
}