...
public interface INormalizable<T>
{
	...
	T centered(T relativeMaxIn);

	T invertedCentered(T relativeMaxIn);

	T between(T minIn, T maxIn);

	T invertedBetween(T minIn, T maxIn);

	T between(T minIn);

	T invertedBetween(T minIn);

	T notNormalized();

	INormalizable<T> value(T valueIn);
}