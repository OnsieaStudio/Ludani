package fr.onsiea.engine.client.input.cursor;
...
public class Cursor implements IButtonTester
{
	...
	private Map<Integer, MouseButtonAction>	buttonsActions;
	...
	public void updateScroll(final CallbackScroll scrollCallbackIn)
	{
	...
	public void update(final long windowHandleIn, final CallbackCursorPosition cursorPositionIn)
	{
	...
	public void input(final int buttonIn, final int actionIn)
	{
		...
		if (action == null)
		{
			action = new MouseButtonAction(buttonIn);
		...
		switch (actionIn)
		{
			case GLFW.GLFW_PRESS:
				...
				if (EnumSubActionType.JUST_PRESSED.equals(action.subType())
						|| EnumSubActionType.PRESSED.equals(action.subType())
						|| EnumSubActionType.PRESSED_FOR_WHILE.equals(action.subType()))
				{
					return;
				}

				action.set(EnumSubActionType.JUST_PRESSED);

				break;

			case GLFW.GLFW_RELEASE:

				if (EnumSubActionType.JUST_RELEASED.equals(action.subType())
						|| EnumSubActionType.RELEASED.equals(action.subType())
						|| EnumSubActionType.RELEASED_FOR_WHILE.equals(action.subType()))
				{
					return;
				}

				action.set(EnumSubActionType.JUST_RELEASED);

				break;

			case GLFW.GLFW_REPEAT:

				if (EnumSubActionType.JUST_REPEATED.equals(action.subType())
						|| EnumSubActionType.REPEATED.equals(action.subType())
						|| EnumSubActionType.REPEATED_FOR_WHILE.equals(action.subType()))
				{
					return;
				}

				action.set(EnumSubActionType.JUST_REPEATED);
	...
	public void end()
	{
		...
		{
			buttonsActionsIterator.next().getValue().action().update();
		}
	...
	@Override
	public IButtonAction buttonAction(final int buttonIn)
	{
	...
	private void buttonsActions(final Map<Integer, MouseButtonAction> buttonsActionsIn)
	{
