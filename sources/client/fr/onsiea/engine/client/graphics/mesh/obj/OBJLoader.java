...
public class OBJLoader implements IOBJLoader
{
	...
	public MeshData loadData(String fileName) throws Exception
	{
		...
		final var			posArr	= new float[vertices.size() * 3];
		var					i		= 0;
		...
		final var	textCoordArr	= new float[vertices.size() * 2];
		final var	normArr			= new float[vertices.size() * 3];
		...
		for (final Face face : faces)
		{
			...
			{
				OBJLoader.processFaceVertex(indValue, textures, normals, indices, textCoordArr, normArr);
			}
		}
	...
	@Override
	public IMesh load(String fileName) throws Exception
	{
		final var				lines		= FileUtils.loadLines(fileName);
