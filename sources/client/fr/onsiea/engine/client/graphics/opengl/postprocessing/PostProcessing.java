...
public class PostProcessing
{
	...
	public PostProcessing(GLMeshManager meshManagerIn, IWindow windowIn, GLShaderManager shaderManagerIn)
			throws Exception
	{
		//this.mesh(meshManagerIn.build("optimizedRectangle",
		//		meshManagerIn.meshBuilder(1).attribBuffer(2).data(PostProcessing.positions()).unbind().vertexCount(4)));
	...
	{
		this.start(/**rendererIn**/
		);
		//this.secondHorizontalBlur().render(colourTextureIn, windowIn, shaderManagerIn);
		//this.secondVerticalBlur().render(this.secondHorizontalBlur().outputTexture(), windowIn, shaderManagerIn);

		//this.brightFilter().render(shaderManagerIn, colourTextureIn, windowIn);

		/**this.horizontalBlur2().render(this.brightFilter().outputTexture(), windowIn, shaderManagerIn);
