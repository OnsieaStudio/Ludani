...
vec3 calcNormal(Material material, vec3 normal, vec2 text_coord, mat4 modelViewMatrix)
{
    ...
    if ( material.hasNormalMap == 1 )
    {
        ...
        newNormal = normalize(newNormal * 2.0f - 1.0f);
...
void main()
{
	...
	vec3 vertexNormal = calcNormal(material, pass_mvVertexNormal, pass_textureCoords, pass_modelView);

    vec4 diffuseSpecularComp = calcDirectionalLight(directionalLight, pass_mvVertexPos, vertexNormal);

    for (int i=0; i<MAX_POINT_LIGHTS; i++)
    {
        if ( pointLights[i].intensity > 0 )
        {
            diffuseSpecularComp += calcPointLight(pointLights[i], pass_mvVertexPos, vertexNormal); 
        }
    }
    ...
    for (int i=0; i<MAX_SPOT_LIGHTS; i++)
    {
        ...
        {
            diffuseSpecularComp += calcSpotLight(spotLights[i], pass_mvVertexPos, vertexNormal);
        }
    }
    diffuseSpecularComp = diffuseSpecularComp;
    
    fragColor =clamp(ambientC * vec4(ambientLight, 1) + diffuseSpecularComp, 0.0f, 1.0f);
	if ( fog.activeFog == 1 ) 
	{
