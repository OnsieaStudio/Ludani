...
void main()
{
    ...
    vec4 diffuseSpecularComp = calcDirectionalLight(directionalLight, pass_mvVertexPos, pass_mvVertexNormal);

    for (int i=0; i<MAX_POINT_LIGHTS; i++)
    {
        if ( pointLights[i].intensity > 0 )
        {
            diffuseSpecularComp += calcPointLight(pointLights[i], pass_mvVertexPos, pass_mvVertexNormal); 
        }
    }
    ...
    for (int i=0; i<MAX_SPOT_LIGHTS; i++)
    {
        ...
        {
            diffuseSpecularComp += calcSpotLight(spotLights[i], pass_mvVertexPos, pass_mvVertexNormal);
        }
    }
